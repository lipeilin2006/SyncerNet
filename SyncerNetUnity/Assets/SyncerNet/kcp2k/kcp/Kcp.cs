...
namespace kcp2k
{
    public class Kcp
    {
        ...
        internal uint xmit;
        internal uint entitylay;       // not a bool. original Kcp has '<2 else' check.
        ...
        public void Flush()
        {
            ...
            uint rtomin = entitylay == 0 ? (uint)rx_rto >> 3 : 0;
            ...
            foreach (Segment segment in snd_buf)
            {
                ...
                else if (Utils.TimeDiff(current, segment.resendts) >= 0)
                {
                    ...
                    {
                        int step = (entitylay < 2) ? segment.rto : rx_rto;
                        segment.rto += step / 2;
                    }
        ...
        //   entitylay : Whether entitylay mode is enabled, 0 is not enabled; 1 enabled.
        ...
        // Normal Mode: ikcp_entitylay(kcp, 0, 40, 0, 0);
        // Turbo Mode： ikcp_entitylay(kcp, 1, 10, 2, 1);
        public void SetNoDelay(uint entitylay, uint interval = INTERVAL, int resend = 0, bool nocwnd = false)
        {
            this.entitylay = entitylay;
            if (entitylay != 0)
            {
                rx_minrto = RTO_NDL;
            }
